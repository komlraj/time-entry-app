...
function CreateEntry(props) {
  ...
  function TimerButton() {
    ...
    if(checkTimer.status == 200) {
      ...
      if((!props.timerData && timer) || (!props.timerData && !timer) || (props.timerData && !timer)) {
        ...
        setState(prevState => ({ ...prevState, ['timerError']: 'Here you can set One Timer At a time.' })); 
      }
    } else {
      setState(prevState => ({ ...prevState, ['timeError']: 'Please select valid time.' })); 
    }
  }
  ...
  return (
		<div className='create-entry'>
			<div>
        <form>
        	<input type='text' name='task' onChange={handleState} placeholder='Task'/>
          ...
          <div>
            <span>
              <input type='time' name='startTime' onClick={handleState} onChange={handleState} />
              <input type='time' name='endTime' onClick={handleState} onChange={handleState} />
            </span>
            {
              (timeError || timerError) ? <label>{timeError || timerError}</label> : ''
            }
          </div>
          <button
            className={timer ? 'btn stop-btn' : 'btn'}
